...
function inputNumber() {
    ...
    //사용자의 입력값을 객체에 추가          
    gameData.answer = +prompt(`숫자를 입력해주세요! [${min}~${max}]`);
    gameData.count++;
    gameData.countdown--;
...
function checkNumber() {
    const {
        ...
        answer,
        countdown
    } = gameData; //값을 쓰기위해 가져오며 선언

    if (secret_num === answer) { //정답시
        alert('장답입니다!  ' + count + '번만에 맞추셨군요!');
        checkCountDown(countdown);
    } else if (secret_num > answer) { //up
        alert('UP!!!');
        gameData.min = answer + 1;
    } else if (secret_num > answer) { //down
        alert('DOWN!!');
...
function alertCountDown(countdown) {
    if (countdown > 0) {
        ...
        alert(`정답기회 모두 소진! 문제를 계속 풀어주세요!ㄴ!`);
    }
}
